<?php
	...
	$result  = $db->ud_getQuery('SELECT * FROM `word_list` w WHERE w.wordID IN (SELECT wordID FROM `ud_user_fav` WHERE userID = '.$_SESSION['userID'].')'.$high_sql_query_and_part.$word_like.' ORDER BY w.word ASC');
