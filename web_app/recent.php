<?php
	...
	$result  = $db->ud_getQuery('SELECT * FROM `word_list` w WHERE w.wordID IN (SELECT wordID FROM `ud_user_history` WHERE userID = '.$_SESSION['userID'].') '.$word_like.' ORDER BY w.word ASC');
